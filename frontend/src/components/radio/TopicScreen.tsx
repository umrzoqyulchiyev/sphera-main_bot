  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-16 gap-4">
        <Loader className="w-10 h-10 text-[#27d9f7] animate-spin" />
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col items-center gap-6">
      ...
      <div className="flex flex-col items-center gap-1 text-center mt-2">
        <span className="font-mono text-[12px] text-[#bbc9cd]/70 tracking-[0.15em] uppercase">
          {tx('level')} {lvl}
        </span>
        <span className="font-mono text-[12px] text-[#27d9f7] tracking-[0.15em] uppercase">
          {tx('stream_active')}
        </span>
      </div>
      ...
      {topic ? (
        <div className="stitch-card-bordered p-5 w-full"
          style={{ background: 'linear-gradient(135deg, rgba(46,168,255,0.1), rgba(124,92,255,0.05))' }}>
          <div className="flex items-center gap-2 mb-3">
            <div className="w-8 h-8 rounded-xl flex items-center justify-center" style={{ background: 'rgba(56,225,255,0.15)' }}>
              <span className="material-symbols-outlined text-[#27d9f7]" style={{ fontSize: 18 }}>auto_awesome</span>
            </div>
            <span className="text-[10px] font-bold tracking-[2px] text-[#bbc9cd]/70 uppercase font-mono">{tx('title')}</span>
          </div>
          <h2 className="text-[19px] font-extrabold text-[#dfe2f1] leading-tight mb-2">{topic.title}</h2>
          {topic.description && (
            <p className="text-[13px] text-[#bbc9cd] leading-relaxed mb-3">{topic.description}</p>
          )}
          <div className="flex items-center gap-2 text-[#27d9f7]">
            ...
            <span className="text-[11px] text-[#859397]">{tx('opinions')}</span>
          </div>
        </div>
      ) : (
        <div className="stitch-card p-8 text-center w-full">
          <div className="w-14 h-14 mx-auto rounded-2xl flex items-center justify-center mb-3" style={{ background: 'rgba(56,225,255,0.08)' }}>
            <span className="material-symbols-outlined text-[#859397]" style={{ fontSize: 28 }}>forum</span>
          </div>
          <p className="text-sm text-[#dfe2f1] font-semibold">{tx('no_topic')}</p>
          <p className="text-xs text-[#859397] mt-1">{tx('wait')}</p>
        </div>
      )}
      ...
      {topic && (
        <div className="stitch-card p-4 w-full flex flex-col gap-3">
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-bold tracking-wide text-[#27d9f7] uppercase font-mono">{tx('your_opinion')}</span>
            <span className="text-[9px] text-[#859397] font-mono">text: 0.001 · voice: 0.005</span>
          </div>
          ...
          <textarea
            ...
            rows={3}
            className="w-full rounded-2xl px-3 py-2.5 text-sm text-[#dfe2f1] resize-none outline-none placeholder:text-[#859397]"
            style={{ background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(56,225,255,0.2)' }}
          />
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              style={{
                ...
                border: `1px solid ${isRecording ? 'rgba(239,68,68,0.5)' : 'rgba(56,225,255,0.2)'}`,
              }}
            >
              <span
                className={`material-symbols-outlined ${isRecording ? 'text-[#ef4444]' : 'text-[#27d9f7]'}`}
                style={{ fontSize: 22, animation: isRecording ? 'pulse-glow 1s infinite' : 'none' }}
              >
                mic
              </span>
            </button>
      ...
      <div className="stitch-card px-4 py-3 flex items-center gap-3 w-full"
        ...
        <span className="material-symbols-outlined text-[#a78bfa] shrink-0" style={{ fontSize: 18 }}>auto_awesome</span>
        <span className="text-[11px] text-[#bbc9cd] leading-relaxed">{tx('ai_info')}</span>
      </div>
