    ...
    <div className="stitch-card p-4"
      ...
      <div className="flex items-center justify-between mb-3">
        {slot.is_live_now ? (
          <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg"
            ...
            <span className="text-[10px] font-bold text-[#ef4444] uppercase tracking-wide">{tx('live_now')}</span>
          </div>
        ) : slot.is_soon ? (
          <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg"
            style={{ background: 'rgba(245,158,11,0.12)', border: '1px solid rgba(245,158,11,0.2)' }}>
            <div className="w-1.5 h-1.5 rounded-full bg-[#f59e0b] animate-pulse" />
            <span className="text-[10px] font-bold text-[#f59e0b] uppercase tracking-wide">{tx('soon')}</span>
          </div>
        ) : (
          <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg"
            style={{ background: 'rgba(94,106,210,0.06)', border: '1px solid rgba(94,106,210,0.1)' }}>
            <Calendar className="w-3 h-3 text-[#8a8f98]" />
            <span className="text-[10px] text-[#8a8f98] uppercase tracking-wide">{tx('scheduled')}</span>
          </div>
        )}
        <span className="text-[10px] text-[#8a8f98]">{slot.duration_min} {tx('duration')}</span>
      </div>

      {/* Title */}
      <div className="text-[15px] font-black text-[#ededef] mb-1 leading-tight">{slot.title}</div>
      {slot.description && (
        <div className="text-[12px] text-[#8a8f98] mb-2 leading-relaxed">{slot.description}</div>
      )}
      ...
      <div className="flex items-center gap-2 mb-3">
        <div className="w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0"
          ...
          <Radio className="w-3 h-3 text-[#5e6ad2]" />
        </div>
        <span className="text-[11px] text-[#8a8f98]">{tx('host')}: </span>
        <span className="text-[11px] font-semibold text-[#ededef]">{hostName}</span>
      </div>

      {/* Countdown (agar live emas va vaqt qolgan bo'lsa) */}
      {!slot.is_live_now && total > 0 && (
        <div className="rounded-xl px-3 py-2.5 mb-3"
          style={{ background: 'rgba(5,5,6,0.5)', border: '1px solid rgba(94,106,210,0.1)' }}>
          <div className="text-[9px] text-[#8a8f98] uppercase tracking-wide mb-1.5">{tx('countdown')}</div>
          <div className="flex items-baseline gap-3">
            {d > 0 && (
              <div className="text-center">
                <div className="text-xl font-black text-[#5e6ad2] tabular-nums"
                  style={{ textShadow: '0 0 15px rgba(94,106,210,0.5)' }}>{d}</div>
                <div className="text-[8px] text-[#8a8f98] uppercase">{tx('days')}</div>
              </div>
            )}
            <div className="text-center">
              <div className="text-xl font-black text-[#5e6ad2] tabular-nums"
                style={{ textShadow: '0 0 15px rgba(94,106,210,0.5)' }}>
                {String(h).padStart(2, '0')}
              </div>
              <div className="text-[8px] text-[#8a8f98] uppercase">{tx('hours')}</div>
            </div>
            <div className="text-[#8a8f98] font-bold mb-2">:</div>
            <div className="text-center">
              <div className="text-xl font-black text-[#5e6ad2] tabular-nums"
                style={{ textShadow: '0 0 15px rgba(94,106,210,0.5)' }}>
                {String(m).padStart(2, '0')}
              </div>
              <div className="text-[8px] text-[#8a8f98] uppercase">{tx('minutes')}</div>
            </div>
            <div className="text-[#8a8f98] font-bold mb-2">:</div>
            <div className="text-center">
              <div className="text-xl font-black text-[#5e6ad2] tabular-nums"
              ...
              <div className="text-[8px] text-[#8a8f98] uppercase">{tx('seconds')}</div>
            </div>
          </div>
        </div>
      )}
      ...
      {slot.share_url && (
        <button onClick={shareSlot}
          ...
          style={{ background: 'rgba(94,106,210,0.08)', border: '1px solid rgba(94,106,210,0.15)', color: '#5e6ad2' }}>
    ...
    <div className="flex flex-col gap-4">
      ...
      {slots.length === 0 ? (
        <div className="stitch-card p-10 text-center">
          <Clock className="w-12 h-12 text-[#8a8f98] mx-auto mb-4" />
          <div className="text-sm font-bold text-[#ededef] mb-1">{tx('no_slots')}</div>
          <div className="text-[11px] text-[#8a8f98]">{tx('no_slots_sub')}</div>
        </div>
      ) : (
