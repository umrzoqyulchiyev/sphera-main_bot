...
from app.core.models import FavoriteAddRequest, FavoriteOut, OkResponse
...
async def add_favorite(payload: FavoriteAddRequest, user: dict = Depends(get_current_user)):
    ...
    if existing:
        raise HTTPException(status_code=409, detail="This item is already in favorites")
...
async def remove_favorite(favorite_id: int, user: dict = Depends(get_current_user)):
    ...
    if result.endswith("0"):
        raise HTTPException(
            status_code=404, detail="Favorite not found or you don't have permission to delete it"
        )
...
async def check_favorite(item_type: str, item_id: int, user: dict = Depends(get_current_user)):
    ...
    return {"is_favorite": row is not None, "favorite_id": row["id"] if row else None}
